...
Uniform_Grid::Uniform_Grid(const Domain& domain, const float divide_length, const std::vector<Vector3>& pos_vectors)
...
//size_t Uniform_Grid::search(const Vector3& pos, size_t* pids) const
//{
...
//
//        if (_num_x_cell < neighbor_gcid_vector.x)
//          continue;
//
//        if (_num_y_cell < neighbor_gcid_vector.y)
//          continue;
//
//        if (_num_z_cell < neighbor_gcid_vector.z)
//          continue;
//
...
std::vector<size_t> Uniform_Grid::search(const Vector3& pos) const
{
...
void Uniform_Grid::update(const std::vector<Vector3>& pos_vectors)
{
  const size_t num_particles = pos_vectors.size();
...
Index_Vector Uniform_Grid::grid_cell_index_vector(const Vector3& v_pos) const
{
...
size_t Uniform_Grid::grid_cell_index(const Vector3& v_pos) const
{
  const float dx = v_pos.x - _domain.x_start;
  const float dy = v_pos.y - _domain.y_start;
  const float dz = v_pos.z - _domain.z_start;
...
size_t Uniform_Grid::grid_cell_index(const Index_Vector& index_vector) const
{
