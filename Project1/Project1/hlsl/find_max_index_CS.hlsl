...
StructuredBuffer<float> value_input : register(t0);
StructuredBuffer<uint>  index_input : register(t1);
...
void main(uint3 Gid : SV_GroupID, uint3 DTid : SV_DispatchThreadID, uint Gindex : SV_GroupIndex)
{
  ...
  {
    const uint index = index_input[data_index];

    shared_value[Gindex]  = value_input[index];
    shared_index[Gindex]  = index;
  }
  else
