...
처음에는 압력을 간단한 수식으로 계산할 수 있어, solver가 단순하고 압력을 매우 빠르게 계산할 수 있다는 장점이 있는 WCSPH를 구현하였지만  $\Delta t$가 매우 작은 값만 가능한 WCSPH의 한계로 인해, PCISPH을 구현 및 적용하였으며 결론적으로 동일 simulation 기준 $\Delta t$를 `10배 증가`시킬 수 있었다.
...
### Read Back 최적화
GPU에서 CPU로 read back이 존재하는 경우, 동기화 비용이 발생하여 계산 시간에 악영향을 준다.
...
만약 read back을 없앨 수 있다면 CPU는 Produce Work를 GPU는 Consume Work를 비동기적으로 수행할 수 있다.

![opt](https://github.com/user-attachments/assets/48f538d1-1a24-4268-979b-c309b787b65e)

따라서, read back을 없애는데 드는 비용이 동기화 비용보다 작다면 read back을 없애는 것이 낫다.
