...
### SPH Scheme
SPH를 이용한 수치해석과정에서 가장 핵심이 되는 부분은 비압축성 가정을 만족하는 압력을 결정하는 방법이다.

압력 결정 방법에 따라 다양한 수치해석기법이 존재하며 이번 프로젝트에서는 linear solver가 필요없는 기법인 Weakly Compsressible SPH(WCSPH)와 Prediction Correction Incompressible SPH(PCISPH)를 사용하였다.

WCSPH는 압력을 간단한 수식으로 계산할 수 있어, solver가 단순하고 압력 계산을 매우 빠르게 계산할 수 있다는 장점이 있지만 $\Delta t$가 매우 작은 값만 가능하다는 한계가 존재한다.

이런 한계점을 극복하기 위해 PCISPH 기법을 적용하였으며, 결론적으로 동일 simulation 기준 $\Delta t$를 `10배 증가`시킬 수 있었다.
...
## GPU 코드 최적화
GPU 코드를 최적화 하기 위해 ID3D11Query를 활용한 계산시간 측정 코드를 작성하였다.

Double Dam Breaking Simulation2를 기준으로 성능 측정을 하였고 최적화 전 Frame당 계산시간은 다음과 같다.
...
### Chunk
기존에 Group당 128개의 Thread를 할당하고 한 thread당 2개의 neighbor particle에 대한 연산을 수행하는 코드가 있었다. 

하지만 일반적으로 neighbor particle의 개수는 약 50개정도임으로 평균적으로 100개에 가까운 Thread가 일을 하지 않는 문제가 있었다.

그렇다고 Thread 개수를 줄이기에는 neighbor particle 개수가 최대 200개가 될 수 있기 때문에 불가능하였다.
